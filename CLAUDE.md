...
- **Page content width:** the layout is **fully fluid** — the AppLayout content container is `w-full min-w-[900px]` (no max cap), so content fills the viewport at any width and scales with the L1 sidebar on monitors of any size. Floored at 900px (desktop console; below ~960px viewport it scrolls horizontally — not tablet/mobile responsive by design). Data pages wrap content in `w-full p-6` (NOT `max-w-[…] mx-auto`) so they fill fluidly. Centred dialogs (ChannelPicker `w-[520px]`, wizard start step `w-[36rem] max-w-[90vw]`) stay intentionally bounded — do not make dialogs fill ultra-wide screens. Sidebar-nav pages (Content, Audiences, Channels, Governance) use `flex flex-1 min-h-0`. Settings (`max-w-[800px]`) and legal pages (`max-w-4xl`) stay intentionally narrow — do not widen.
